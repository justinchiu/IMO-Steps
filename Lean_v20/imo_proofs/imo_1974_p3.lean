  ...
  have h₀: ∑ x ∈ range (n + 1), ↑((2 * n + 1).choose (2 * x + 1)) * k ^ (2 * x + 1) =
    ...
    have h₀₁: ∑ x ∈ fs₃, f₀ (2 * x + 1) = ∑ x ∈ (fs₀), -1 * f₁ x := by
      ...
      . intros b hb₀
        ...
        have h₀ : f₀ = fun (i : ℕ) ↦ (↑(((2 : ℕ) * n + (1 : ℕ)).choose i) : ℝ) * k ^ i := by rfl
        have h₁ : f₁ = fun (i : ℕ) ↦ (↑(((2 : ℕ) * n + (1 : ℕ)).choose i) : ℝ) * (-k) ^ i := by rfl
        rw [h₀, h₁]
        simp
        rw [neg_eq_neg_one_mul k, mul_pow, pow_add (-1 : ℝ) 1]
        rw [hb₁, mul_one, pow_one]
        bound
    exact h₀₁
  have h₁: ∑ x ∈ range (n + 1), ↑((2 * n + 1).choose (2 * x)) * k ^ (2 * x) =
    ∑ x ∈ fs₁, ↑((2 * n + 1).choose (x)) * (-k) ^ (x) := by
...
theorem imo_1974_p3
  ...
  . rw [h₃] at h₄₁
    ...
    have hc₃: a ^ 2 ≡ 2 [MOD 5] := by
      exact Nat.ModEq.add_left_cancel'  3 hc₁
