...
/- Let $a_1, a_2,\\cdots, a_n$ be real constants, $x$ a real variable,
  and $f(x)=\\cos(a_1+x)+\\frac{1}{2}\\cos(a_2+x)+\\frac{1}{4}\\cos(a_3+x)+\\cdots+\\frac{1}{2^{n-1}}\\cos(a_n + x).$
  Assume that $f(x_1)=f(x_2)=0,$.
  Further assume $\\cos(a_1)+\\frac{1}{2}\\cos(a_2)+\\frac{1}{4}\\cos(a_3)+\\cdots+\\frac{1}{2^{n-1}}\\cos(a_n)$ is not equal to zero.
  Prove that $x_2-x_1=m\\pi$ for some integer $m$. -/
...
theorem imo_1969_p2
  ...
  (h₂ : f m = 0)
  (h₃ : f n = 0)
  (h₄: ∑ i ∈ Finset.range k, ((Real.cos (a i)) / (2^i)) ≠ 0) :
  ∃ t : ℤ, m - n = t * π := by
  ...
  have h₆: (∃ x, (f x = 0 ∧ cos x = 0)) → ∀ y, f y = Ccos * cos y := by
    ...
    have g₁: Finset.sum (Finset.range k) (fun i => (((sin (a i)) / (2 ^ i)))) = 0 := by
      rw [h₅ x, hx₁, mul_zero] at hx₀
      rw [@sub_eq_self] at hx₀
      apply mul_eq_zero.mp at hx₀
      cases' hx₀ with hx₂ hx₃
      . exact hx₂
      . exfalso
