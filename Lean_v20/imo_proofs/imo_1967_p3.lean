...
/- Let $k, m, n$ be natural numbers such that $m+k+1$ is a prime greater than $n+1$. Let $c_s=s(s+1).$
  Prove that the product $(c_{m+1}-c_k)(c_{m+2}-c_k)\\cdots (c_{m+n}-c_k)$ is divisible by the product $c_1c_2\\cdots c_n$. -/
...
theorem imo_1967_p3
  ...
  have h₄: ∏ i ∈ Finset.Icc 1 n, c i = n.factorial * (n + 1)! := by
  ...
  rw [h₄, h₅]
  by_cases hk₀: k ≤ m
  . have h₆: ∀ n m, 0 < n → n ! ∣ (∏ i ∈ Finset.Icc 1 n, (m + i)) := by
      intros s t hs₀
      have h₆₀ : ∏ i ∈ Finset.Icc (1 : ℕ) s, (t + i) = (t + s)! / t ! := by
        ...
        . refine Nat.le_induction ?_ ?_ s hs₀
          ...
          . simp
            ...
            rw [← add_assoc]
            rw [Nat.factorial_succ]
            exact Nat.mul_comm (t + d)! (t + d + (1 : ℕ))
      have h₆₁ : (∏ i ∈ Finset.Icc 1 s, (t + i)) = s ! * Nat.choose (t + s) (t) := by
        rw [h₆₀]
        have hk₁ : t ≤ t + s := by exact Nat.le_add_right t s
        rw [Nat.choose_eq_factorial_div_factorial hk₁]
        rw [Nat.add_sub_cancel_left]
        have hk₂: ((t)! * s !) ∣ (t + s)! := by exact factorial_mul_factorial_dvd_factorial_add t s
        have ht₁: 0 < s ! := by exact factorial_pos s
        rw [← Nat.mul_div_assoc _ hk₂, mul_comm]
        rw [Nat.mul_div_mul_right _ _ ht₁]
      rw [h₆₁]
      exact Nat.dvd_mul_right s ! ((t + s).choose t)
    have h₇: n ! ∣ (∏ i ∈ Finset.Icc 1 n, (m + i - k)) := by
      have h₇₀: ∀ i, m + i - k = m - k + i := by
        intro i
        exact Nat.sub_add_comm hk₀
      simp_rw [h₇₀]
      exact h₆ n (m - k) h₀.2.2
    have h₈: (n + (1 : ℕ))! ∣ (∏ i ∈ Finset.Icc (1 : ℕ) n, (m + i + k + (1 : ℕ))) := by
      have h₈₀: (n + (1 : ℕ))! ∣ (k + m + 1) * (∏ i ∈ Finset.Icc (1 : ℕ) n, (m + i + k + (1 : ℕ))) := by
        have hn₀ : 0 < n + 1 := by exact zero_lt_succ n
