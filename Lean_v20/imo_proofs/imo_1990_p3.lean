...
/- Consider all integers $n > 1$ such that $\\frac{2^n+1}{n^2}$ is an integer.
  Prove that the only 3 can satisfy this condition. -/
  ...
  . push_neg at hn₀
    ...
    have h₄: ∀ p, Nat.Prime p → 3 ≤ p → p ∣ 2 ^ (p - 1) - 1 := by
      ...
      have hp₄: 2 ^ (p - 1) ≡ 1 [ZMOD p] := by
        ...
        refine Int.isCoprime_iff_nat_coprime.mpr ?_
        norm_cast
      ...
      left
      rw [LucasLehmer.Int.coe_nat_two_pow_pred]
