...
/- Let $f(1), f(2), f(3), ..., f(n), ...$, be a sequence of distinct positive integers.
  Prove that for all positive natural numbers $n$ we have: $\\sum_{k=1}^{n} f(k)k^{-2} \\geq \\sum_{k=1}^{n} k^{-1}.$ -/
...
lemma aux_1
  ...
  have hhf₀ : f₀ = fun k:ℕ => 1 / (k:ℝ) ^ 2 := by rfl
  ...
  have hρ: ∃ ρ: Equiv.Perm ℕ, (∀ a ∈ Icc 1 n, f₂ a = f₃ (ρ a)) ∧ ∀ a ∉ Icc 1 n, ρ a = a := by
    ...
    have gg₁₅: ∀ x y, x ∈ s → y ∈ s → (f₄ x = f y ↔ f₈ x = y) := by
      ...
      have hy₂: y - 1 < (List.map f sl).length := by
        rw [List.length_map f, hs₀, List.length_range', ← gg₁₁]
        exact hy₁
